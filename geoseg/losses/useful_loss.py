...
class BiSeNetV1Loss(nn.Module):

    def __init__(self, ignore_index=255):
        super().__init__()
        self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                                   DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
        self.aux_loss1 = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.aux_loss2 = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.aux_loss3 = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.aux_loss4 = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
    ...
    def forward(self, logits, labels):
        ...
        elif self.training and len(logits)==5:
            logit_main, logit_aux1, logit_aux2, logit_aux3, logit_aux4 = logits
            loss = self.main_loss(logit_main, labels) + 0.1 * self.aux_loss1(logit_aux1, labels) + 0.1 * self.aux_loss2(logit_aux2, labels) + 0.1 * self.aux_loss1(logit_aux3, labels) + 0.1 * self.aux_loss2(logit_aux3, labels)
        else:
            loss = self.main_loss(logits, labels)
