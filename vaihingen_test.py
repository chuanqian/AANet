...
def main():
    ...
    precision = np.nanmean(evaluator.Precision()[:-1])
    recall = np.nanmean(evaluator.Recall()[:-1])
    OA = evaluator.OA()
    ...
    print('F1:{:.4f}, OA:{:.4f}, mIOU:{:.4f}, precision:{:.4f}, recall:{:.4f}'.format(np.nanmean(f1_per_class[:-1]), OA, np.nanmean(iou_per_class[:-1]), precision, recall))
    t0 = time.time()
    for inp in results:
        img_writer(inp)
